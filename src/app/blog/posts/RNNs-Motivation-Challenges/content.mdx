...
>A special case of the general $IIR$ formulation is that of momentum:$$y[t] = αy[t-1] + (1-α)x[t]$$ The decay factor α determines how much historical information is retained
> This gives us intuition about the "memory" capacity of our network


We can see that we are processing inputs sequentially,  When there are inputs needed to be processed sequentially, $IIR$ can be useful.
...
As always, let's first try to design and understand a linear version of $IIR$ filters and then try to generalize to non-linear case.
...
> This is a compact way to write the Kalman filter, this is different from the textbook definition that usually works in a predict and then update fashion.
...
+ We also have that,  $\vec{h}_{t+1|t} = A\vec{h}_{t|t}$ this is the update phase (*prior belief*), we assumed that the noise is dominated by the update.
+ The matrix $B$ incorporates many common matrices used in the standard Kalman filter definition.

The reason we opted for this formulation is because in the standard (non-learnable) formulation, we know what each matrix represents and they are usually known depending on the *dynamics* of the physical system we are trying to model (Mainly known probability matrices).

Here, since we are learning these parameters (Through Gradient Descent), we can combine those matrices, as it's technically the same thing if we learn them separately.

> A thought provoking concept to look at, is the relationship between the gaussian noise we are supposing, the fact that we are trying to minimize variance, the squared loss that the minimization induces and how that can be derived with MLE (Same as in OLS).

Let's try to intuitively discuss what does those two equations really say:
...
1.  We have traces of train data  $(\vec{h_{t,j}}, \vec{x}_{t,j}  )^{n_j}_{j=0}$  for the $js$ being $1, 2, . . . , m$. Notice that $n_j$ is  the length and we would want to learn the weights $A$ and $B$.
	- *Advantages:* More stable training, faster convergence
	- *Challenges:* Requires labelled hidden states, may not be available in practice
	- *Applications:* System identification, physical system modelling
...
2.  We only have access to $(\vec{x}_{t,j})^{n_j}_{j=0}$, and we would want to learn the weights $A$, $B$ and $c$.
	...
	- *Applications:* Sequence prediction, language modelling


For the first case, the network can look like this:

![[Pasted image 20250220125912.png]]
...
In real systems, we can directly find a closed-form solution, here we are learning the weights. This is similar to $OLS$ where we can use the closed-form solution or learn with $GD$.
...
![[Pasted image 20250220134810.png]]
...
1. We change the internal structure of each weight box in the $RNN$ by replacing the current linear combinations with an $MLP$. The network inside can be arbitrarily wide and/or deep.
   ![[Pasted image 20250220135621.png]]
2. We use layers of simpler $RNNs$. Treat each $RNN$ as a filter, we compose filters together. It is the same way that convolution network gets deeper. We can put an output layer above, and the inputs are on the ground. The gradient could flow in two directions.
   ![[Pasted image 20250220135843.png]]
...
![[Pasted image 20250220145454.png]]
...
![[Pasted image 20250220155045.png]]
